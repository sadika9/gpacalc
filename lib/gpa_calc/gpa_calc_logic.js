...
function updateIncludeInOverallGPAState(object) {
    ...
    var courseCode = object.getElementsByTagName('TD')[2].innerHTML;
    for (var i = 0; i < global_student.enrolledCourses.length; ++i) {
        if (global_student.enrolledCourses[i].course.code === courseCode) {
            global_student.enrolledCourses[i].include_in_overall_gpa = checkBoxState;
            
            /* update subject gpa states when overall gpa check box changed */
            if (checkBoxState) { 
                global_student.enrolledCourses[i].include_in_subject_gpa = [].concat(global_student.enrolledCourses[i].course.principal_subject_area);
