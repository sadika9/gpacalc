...
function addSubject() {
    ...
    global_student.addSubject(subject.code, subject.subject);
...
function removeSubject(object) {
    ...
    var subj = object.getElementsByTagName('TD')[1].innerHTML;
    var code = object.getElementsByTagName('TD')[2].innerHTML;
    
    global_student.removeSubject(code);
...
function updateExistingCoursesSubjectGPAStates(action, subject) {
    if (action === "add_subject") {
        for (var i = 0, nStuCourses = global_student.enrolledCourses.length; i < nStuCourses; ++i) {
            for (var j = 0, nSubjArea = global_student.enrolledCourses[i].course.principal_subject_area.length; j < nSubjArea; ++j) {
                if (global_student.enrolledCourses[i].course.principal_subject_area[j] === subject.code) {
                    global_student.enrolledCourses[i].include_in_subject_gpa.push(subject.code);
                }
            }
        }
    } else {
        for (var i = 0, nStuCourses = global_student.enrolledCourses.length; i < nStuCourses; ++i) {
            var index = global_student.enrolledCourses[i].include_in_subject_gpa.indexOf(subject.code);
            if (index > -1) {
                global_student.enrolledCourses[i].include_in_subject_gpa.splice(index, 1);
            }
        }
    }
}
...
function addCourse() {
    ...
    // multiple entries of same course can be in different years(repeat) but not in same year
    ...
    for (var i = 0; i < global_courses.length; ++i) {
        if (global_courses[i].code === courseCode) {
            course = courseFromGlobalCourse(global_courses[i]);
            break;
        }
    }
    ...
    var repeat = document.getElementById('repeat_checkbox').checked;
    var include_in_overall_gpa = document.getElementById('include_in_overall_gpa_checkbox').checked;

    global_student.addCourse(year, semester, course, repeat, grade, include_in_overall_gpa);
...
function editCourse(object) {
    ...
    document.getElementById("edit_student_courses_modal_label_show").innerHTML = eC.course.code + " - " + eC.course.title;
    document.getElementById("edit_student_courses_modal_course_code").innerHTML = eC.course.code;
    document.getElementById("edit_student_courses_modal_year").innerHTML = eC.year;
    document.getElementById("edit_student_courses_modal_credits").value = eC.course.credits;
    document.getElementById("edit_student_courses_modal_semester").value = eC.semester;
    document.getElementById("edit_student_courses_modal_grade").value = eC.grade;
    document.getElementById("edit_student_courses_modal_repeat").checked = eC.repeat;
...
function saveEditedStudentCourseDetail() {
    ...
    eC.semester = document.getElementById("edit_student_courses_modal_semester").value;
    eC.course.credits = Number(document.getElementById("edit_student_courses_modal_credits").value);
    eC.grade = document.getElementById("edit_student_courses_modal_grade").value;
    eC.repeat = document.getElementById("edit_student_courses_modal_repeat").checked;
...
function updateCourseTables() {
    ...
    for (var i = 0; i < global_student.enrolledCourses.length; ++i) {
        var tbody = document.getElementById("student_courses_year" + global_student.enrolledCourses[i].year + "_table").getElementsByTagName("TBODY")[0];
        var rowCount = tbody.rows.length;
        var row = tbody.insertRow(rowCount);
        ...
        row.insertCell(1).appendChild(document.createTextNode(global_student.enrolledCourses[i].semester));
        row.insertCell(2).appendChild(document.createTextNode(global_student.enrolledCourses[i].course.code));
        row.insertCell(3).appendChild(document.createTextNode(global_student.enrolledCourses[i].course.title + (global_student.enrolledCourses[i].repeat ? " (R)" : "")));
        row.insertCell(4).appendChild(document.createTextNode(global_student.enrolledCourses[i].course.credits));
        row.insertCell(5).appendChild(document.createTextNode(global_student.enrolledCourses[i].grade));

        var cell = row.insertCell(6);
        var eleChBx = document.createElement("input");
        eleChBx.type = "checkbox";
        eleChBx.value = "overall_gpa_column";
        eleChBx.checked = global_student.enrolledCourses[i].include_in_overall_gpa;
        eleChBx.onclick = function () {
            updateIncludeInOverallGPAState(this)
        };
        cell.appendChild(eleChBx);

        for (var j = 0; j < global_student.subjects.length; ++j) {
            var cell = row.insertCell(j + 7);
            var eleChBx = document.createElement("input");
            eleChBx.type = "checkbox";
            eleChBx.value = global_student.subjects[j].code;
            eleChBx.onclick = function () {
                updateIncludeInSubjectGPAState(this)
            };
            eleChBx.checked = false;
            eleChBx.disabled = !global_student.enrolledCourses[i].include_in_overall_gpa;
            
            if (!eleChBx.disabled) {
                for (var k = 0; k < global_student.enrolledCourses[i].include_in_subject_gpa.length; ++k) {
                    var subCode = global_student.enrolledCourses[i].include_in_subject_gpa[k];
                    if (subCode === global_student.subjects[j].code) {
...
function updateIncludeInOverallGPAState(object) {
    ...
    eC.include_in_overall_gpa = checkBoxState;
            
    /* update subject gpa states when overall gpa check box changed */
    if (checkBoxState) { // populate include_in_subject_gpa with subjects from course.principal_subject_area, except "Foundation"
        eC.include_in_subject_gpa = [].concat(eC.course.principal_subject_area);
        var idx = global_student.enrolledCourses[i].include_in_subject_gpa.indexOf("Foundation");
        if (idx > -1) {
            global_student.enrolledCourses[i].include_in_subject_gpa.splice(idx, 1);
        }
    }
    else { /* need to remove current course from all subject gpas if user removes  the course from overall gpa */
        global_student.enrolledCourses[i].include_in_subject_gpa = [];
    }
...
function updateIncludeInSubjectGPAState(object) {
    ...
    if (checked) {
        eC.include_in_subject_gpa.push(subjCode);
    } 
    else {
        var index = eC.include_in_subject_gpa.indexOf(subjCode);
        if (index > -1) eC.include_in_subject_gpa.splice(index, 1);
    }
...
function totalCredits(year) {
    ...
    for (var i = 0, nStuCourses = global_student.enrolledCourses.length; i < nStuCourses; ++i) {
        var stuCourse = global_student.enrolledCourses[i];
