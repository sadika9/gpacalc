...
Student.prototype.sortSubjects = function() {
...
Student.prototype.addCourse = function(year, semester, course, repeat, grade, include_in_overall_gpa, include_in_subject_gpa) {   
    ...
    this.sortEnrolledCourses();
}
   
Student.prototype.removeCourse = function(year, courseCode) {
    var index = -1;
    for (var i = 0, n = this.enrolledCourses.length; i < n; ++i) {
        var eC = this.enrolledCourses[i];
        if (eC.year === year && eC.course.code === courseCode) {
            index = i;
            break;
        }
    }
...
Student.prototype.sortEnrolledCourses = function() {
...
Student.prototype.getCourseRef = function(year, courseCode) {
    for (var i = 0, n = this.enrolledCourses.length; i < n; ++i) {
        var eC = this.enrolledCourses[i];
        if (eC.year === year && eC.course.code === courseCode) {
            return eC;
...
Student.prototype.setIncludeInOverallGpa = function(year, courseCode, state) {
    this.getCourseRef(year, courseCode).include_in_overall_gpa = state;
}

Student.prototype.setIncludeInSubjectGpa = function(year, courseCode, subjectCode, includeSubject) {
    var eC = this.getCourseRef(year, courseCode);
    
    if (includeSubject) {
        eC.include_in_subject_gpa.push(subjectCode);
    } 
    else {
        var index = eC.include_in_subject_gpa.indexOf(subjectCode);
        if (index > -1) {
            eC.include_in_subject_gpa.splice(index, 1);
        }
    }
}
