...
$('#student_subjects_table').on('click', '.delete_row_button', function() {
    ...
    tr.fadeOut(400, function(){
        ...
        notify('Remove subject: ' + $('td:nth-child(2)', tr).text(), 'info');
...
$('#student_courses_table').on('click', '.delete_row_button', function() {
    ...
    tr.css('background-color','#CA3C3C');
    tr.fadeOut(400, function(){
        var year = Number($('td:nth-child(2)', tr).text());
        var courseCode = $('td:nth-child(4)', tr).text();
...
function drawSubjectsTable() {
    ...
    for (var i = 0, n = global_student.subjects.length; i < n; ++i) {
        ...
        rows += '<tr><td><button class="btn btn-danger btn-xs delete_row_button"><span class="glyphicon glyphicon-remove"></span></button></td>' +
...
function drawCoursesTable() {
    ...
    // headings
    var thead = '<thead><th></th><th class="hidden">Year</th><th>Semester</th><th>Code</th><th>Title</th><th>Credits</th><th>Grade</th><th>Overall GPA</th>';
    ...
    for (var year = 1; year <= 4; ++year) { // year loop start
        ...
        for (var i = 0, n = global_student.enrolledCourses.length; i < n; ++i) { // course loop start
            ...
            rows += '<tr><td><button class="btn btn-primary btn-xs edit_row_button"><span class="glyphicon glyphicon-edit"></span></button> ' +
                    '<button class="btn btn-danger btn-xs delete_row_button"><span class="glyphicon glyphicon-remove"></span></button></td>' +
                    '<td class="hidden">' + eC.year + '</td>' +  '<td>' + eC.semester + '</td>' +  '<td>' + eC.course.code + '</td>' +  
        ...
        var tbody = '<tbody><tr class="year_row"><td colspan=' + (7 + nSubjects) + '>Year ' + year + '</td></tr>' + rows + '</tbody>'
