...
for (var i = 0, nSubs = subjects.length; i < nSubs; ++i) {
    // adding subjects to main_subject combo
    var x1 = document.createElement("OPTION");
    x1.setAttribute("value", subjects[i].code);

    var t1 = document.createTextNode(subjects[i].subject);
    x1.appendChild(t1);
    ...
    x2.setAttribute("value", subjects[i].code);

    var t2 = document.createTextNode(subjects[i].subject);
    x2.appendChild(t2);
...
function filterCourses() {
    ...
    if (subjectCode === "ALL") {
        ...
        for (var i = 0; i < courses.length; ++i) {
            var x = document.createElement("OPTION");
            x.setAttribute("value", courses[i].code);

            var t = document.createTextNode(courses[i].code + " - " + courses[i].title);
            x.appendChild(t);
            document.getElementById("course").appendChild(x);
        }
    } else {
        ...
        for (var i = 0; i < courses.length; ++i) {
            if (courses[i].code.indexOf(subjectCode) > -1) {
                var x = document.createElement("OPTION");
                x.setAttribute("value", courses[i].code);
...
function courseSelectionChanged() {
    ...
    for (var i = 0; i < courses.length; ++i) {
        if (courses[i].code === courseCode) {
            ...
            element.checked = !courses[i].exclude_from_gpa;
        }
    }
}
